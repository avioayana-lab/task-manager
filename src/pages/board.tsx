        ...
        <div ref={setNodeRef} style={{ ...style, background: '#fff', padding: '14px', borderRadius: '10px', marginBottom: '10px', cursor: 'grab', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', borderLeft: '4px solid ' + priorityColor }} {...attributes} {...listeners}>
            ...
            {task.tags.length > 0 && (
                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '6px' }}>
                    {task.tags.map((tag) => (
                        <span key={tag} style={{ background: '#ede9fe', color: '#7c3aed', padding: '2px 8px', borderRadius: '20px', fontSize: '11px' }}>{tag}</span>
                    ))}
                </div>
            )}
            {task.deadline && (
                <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '6px' }}>📅 {new Date(task.deadline).toLocaleDateString()}</div>
            )}
            {task.assignee && (
            ...
            >
                ...
                <button onClick={() => onDelete(task.id)} style={{ padding: '3px 8px', fontSize: '11px', borderRadius: '6px', border: '1px solid #fee2e2', cursor: 'pointer', background: '#fff5f5', color: '#ef4444' }}>🗑️</button>
            </div>
        </div>
    )
}
...
function Board() {
    ...
    const boardTasks = tasks.filter((t) => {
        ...
        return matchBoard && matchSearch && matchPriority && matchTag && matchAssignee
    })
    ...
    return (
        <div style={{ padding: '30px' }}>
            ...
            <h1 style={{ fontSize: '28px', marginBottom: '20px', color: '#4f46e5' }}>🗂️ Доска</h1>
            <div style={{ display: 'flex', gap: '10px', marginBottom: '24px', flexWrap: 'wrap', background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                <input
            ...
            <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <div style={{ display: 'flex', gap: '20px' }}>
                    {columns.map((col) => {
                        ...
                        return (
                            <div key={col.id} style={{ flex: 1, background: '#f8fafc', padding: '16px', borderRadius: '12px', minHeight: '300px', borderTop: '4px solid ' + col.color }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                    <h2 style={{ fontSize: '16px', color: col.color }}>{col.title}</h2>
                                    <span style={{ background: col.color, color: '#fff', borderRadius: '20px', padding: '2px 10px', fontSize: '12px' }}>{colTasks.length}</span>
                                </div>
                                <button onClick={() => handleAddTask(col.id)} style={{ width: '100%', marginBottom: '12px', padding: '8px', borderRadius: '8px', border: '2px dashed #e5e7eb', cursor: 'pointer', background: 'transparent', color: '#9ca3af' }}>
                                    + Добавить задачу
                                </button>
